...
node_t* genNode(void)//-----------------------------------------------------------------------list구조체의 껍데기 만들기
{
	node_t* ndPtr;
...
*/
void* list_genList(void)//---------------------------------------------------------------------------list 구조체 껍데기 만듦
{
...
int list_addTail(void* obj, void* list)
{
	...
	{
		printf("[ERROR] Failed to do addTail : list pointer is NULL or object indicates NULL! (%p, %p)\n", list, obj);
		return -1;
	}
...
int list_addNext(void* obj, void* nd)
{
	...
	{
		printf("[ERROR] Failed to do addNext : node pointer is NULL or object indicates NULL! (%p, %p)\n", ndPtr, obj);
		return -1;
	}
...
int list_len(void* list)
{
	...
	if (listPtr == NULL)
	{
		printf("[ERROR] Failed to calculate list length : list pointer is NULL! (%p)\n", list);
		return -1;
	}
...
void* list_getNextNd(void* nd)
{
	...
	if (ndPtr == NULL)
	{
		printf("[ERROR] Failed to get next node : node pointer is NULL!(%p)\n", nd);
		return NULL;
	}
...
void* list_getIndexNd(int index, void* list)
{
	...
	if (listPtr == NULL)
	{
		printf("[ERROR] Failed to get the node with index : list pointer is NULL! (%p)\n", listPtr);
		return NULL;
	}
...
void* list_srchNd(int (*matchFunc)(void* obj, void* cond), void* cond, void* list)
{
	...
	if (listPtr == NULL)
	{
		printf("[ERROR] search node fail : list pointer is NULL! (%p)\n", listPtr);
		return NULL;
	}
...
int list_repeatFunc(void (*func)(void* obj), void* list)
{
	...
	{
		printf("[ERROR] Repeat function fail : list pointer or func is NULL! (%p %p)\n", list, func);
		return -1;
	}
...
void* list_getNdObj(void* nd)
{
	...
	if (ndPtr == NULL)
	{
		printf("[ERROR] failed to get object : node pointer is NULL! (%p)\n", nd);
		return NULL;
	}
